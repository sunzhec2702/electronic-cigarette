...
#include "oledfont_sh1107.h"
#include "oled_content_muxlock.h"

#define I2C_OWN_ADDRESS7    0x78  // don't care
...
void Write_IIC_Command(unsigned char IIC_Command)
{
    ...
    IIC_Wait_Ack();
    Write_IIC_Byte(0x80);			//write command
    IIC_Wait_Ack();
...
void OLED_Display_On(void)
{
    ...
    OLED_WR_Byte(0X81,OLED_CMD);  //DCDC ON
...
{
    OLED_WR_Byte(0X8D,OLED_CMD);  //SET DCDC命令
    OLED_WR_Byte(0X80,OLED_CMD);  //DCDC OFF
...
void OLED_SHOW_SPACE(u16 x, u8 y, u8 size, u8 repeat)
{
    ...
    for (i = 0; i < repeat; i++) {
        ...
        case 16:
            ...
            break;
        case 24:
            OLED_DrawBMP(x + i, y, x + i + 1, y + 2, SPACEF1x16);
            OLED_DrawBMP(x + i, y + 2, x + i + 1, y + 3, SPACEF1x8);
            break;
        case 32:
            OLED_DrawBMP(x + i, y, x + i + 1, y + 2, SPACEF1x16);
            OLED_DrawBMP(x + i, y + 2, x + i + 1, y + 4, SPACEF1x16);
            break;
        }
    }
}
